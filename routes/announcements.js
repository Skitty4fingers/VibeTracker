...
router.post('/', async (req, res) => {
    try {
        ...
        if (pinned) {
            const pinnedCount = await db.execute('SELECT COUNT(*) as cnt FROM Announcement WHERE pinned = 1');
        ...
        await db.execute({
            ...
            args: [title.trim(), body.trim(), published ? 1 : 0, pinned ? 1 : 0],
        });

        const maxResult = await db.execute('SELECT MAX(id) as id FROM Announcement');
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const existing = await getRow(db, 'SELECT * FROM Announcement WHERE id = ?', [id]);
        ...
        await db.execute({ sql: `UPDATE Announcement SET ${sets.join(', ')} WHERE id = ?`, args: params });

        const row = await getRow(db, 'SELECT * FROM Announcement WHERE id = ?', [id]);
